"""
Update Charts & Exhibits Hard drills with proper chart interpretation questions
"""
import sys
import asyncio
...
# Proper Hard-level Charts & Exhibits questions
PROPER_HARD_DRILLS = {
    "ce-h-4": {
        "id": "ce-h-4",
...
async def update_hard_drills():
    """Update Hard drills with proper chart interpretation questions"""
    
    client = AsyncIOMotorClient(MONGO_URL)
    db = client[DB_NAME]
    
    print("🔧 Updating Charts & Exhibits Hard drills with proper questions...")
    ...
    for drill_id, drill_data in PROPER_HARD_DRILLS.items():
        ...
        if result.modified_count > 0:
            print(f"  ✓ Updated {drill_id}: {drill_data['title']}")
            updated += 1
        else:
            print(f"  ⚠ No changes for {drill_id}")
    
    print(f"\n✅ Updated {updated} Hard drills with proper chart questions")
    
    # Verify a drill
    sample = await db.ai_drills.find_one({"id": "ce-h-4"}, {"_id": 0, "questions": 1})
    if sample:
        print(f"\n📋 Sample question from ce-h-4:")
...
if __name__ == "__main__":
    asyncio.run(update_hard_drills())
