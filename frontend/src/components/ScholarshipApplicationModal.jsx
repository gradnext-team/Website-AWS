...
import { X, Upload, Loader2, CheckCircle, Linkedin, Mail, Phone, Building2, Calendar, Image } from 'lucide-react';
import { Button } from './ui/button';

const BACKEND_URL = process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001';

const ScholarshipApplicationModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.interview_company.trim()) {
    ...
    if (!proofFile) {
      setError('Please upload a screenshot as proof of your interview invite');
      return;
    }
    if (!formData.reason_for_applying.trim()) {
      setError('Please tell us why you want to apply for the scholarship');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/forms/scholarship-application`, {
        method: 'POST',
        body: submitData
      });
      ...
      >
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-white">Apply for Scholarship</h2>
              <p className="text-sm text-white/70 mt-1">For candidates with interview invites</p>
            </div>
            <button
              onClick={onClose}
        ...
        {isSuccess ? (
          <div className="p-8 text-center">
            ...
            <p className="text-slate-600">Thank you for applying for the scholarship. Our team will review your application and get back to you soon.</p>
          </div>
        ) : (
          /* Form */
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--gn-rhino)' }}>
                Which company do you have an interview from? <span className="text-red-500">*</span>
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder="e.g., McKinsey, BCG, Bain, Kearney"
                  className="w-full pl-11 pr-4 py-3 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 transition-all"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--gn-rhino)' }}>
                Screenshot of Interview Invite <span className="text-red-500">*</span>
              </label>
              <p className="text-xs text-slate-500 mb-3">Upload proof of your interview invitation (image or PDF)</p>
              ...
              <div 
                ...
                style={{ borderColor: proofFile ? 'var(--gn-periwinkle)' : '#e2e8f0' }}
              >
                <input
                  type="file"
                  accept="image/*,.pdf"
                ...
                {proofFile ? (
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-sm font-medium" style={{ color: 'var(--gn-rhino)' }}>{proofFile.name}</span>
                  </div>
                ) : (
                  <>
                    <Image className="w-6 h-6 mx-auto mb-2 text-slate-400" />
                    <p className="text-sm text-slate-600">
                      <span className="font-medium" style={{ color: 'var(--gn-periwinkle)' }}>Click to upload</span> or drag and drop
                    </p>
                    <p className="text-xs text-slate-400 mt-1">PNG, JPG, GIF, WebP or PDF (max 10MB)</p>
                  </>
                )}
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: 'var(--gn-rhino)' }}>
                Why do you want to apply for the scholarship? <span className="text-red-500">*</span>
              </label>
              <textarea
                name="reason_for_applying"
                value={formData.reason_for_applying}
                onChange={handleInputChange}
                placeholder="Tell us about your situation and why you need the scholarship..."
                rows={4}
            ...
            <p className="text-xs text-center text-slate-500">
              By submitting, you agree to be contacted by our team regarding your scholarship application.
            </p>
          </form>
        )}
      </div>
