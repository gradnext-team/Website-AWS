...
const PinnacleApplicationsTab = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/forms/pinnacle-applications${statusQuery}`, {
        credentials: 'include'
      });
  ...
  const updateStatus = async (applicationId, newStatus) => {
    try {
      ...
      const res = await fetch(`${BACKEND_URL}/api/forms/pinnacle-applications/${applicationId}`, {
        method: 'PATCH',
  ...
  const downloadCV = async (applicationId, filename) => {
    try {
      const res = await fetch(`${BACKEND_URL}/api/forms/pinnacle-applications/${applicationId}/cv`, {
        credentials: 'include'
      });
      ...
      a.href = url;
      a.download = data.filename || filename || 'cv.pdf';
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200" style={{ backgroundColor: 'var(--gn-grey-lightest)' }}>
                ...
                <th className="text-left p-4 font-semibold text-sm" style={{ color: 'var(--gn-rhino)' }}>Education</th>
                <th className="text-left p-4 font-semibold text-sm" style={{ color: 'var(--gn-rhino)' }}>Interview</th>
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <p className="text-sm text-slate-700">{app.undergrad_university}</p>
                            {app.postgrad_university && (
                              <p className="text-xs text-slate-500">{app.postgrad_university}</p>
                            )}
                          </div>
                        </div>
                      </td>
                        ...
                        {app.has_interview ? (
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">{formatDate(app.interview_date).split(',')[0]}</p>
                            </div>
                          </div>
                        ) : (
                          <span className="text-sm text-slate-400">No interview scheduled</span>
                        )}
                      </td>
                      <td className="p-4">
...
export default PinnacleApplicationsTab;
